...
class WXR_Parser_Regex {
	...
	public function fopen( $filename, $mode = 'r' ) {
		if ( $this->has_gzip && ( substr( $filename, -3 ) == '.gz' ) ) {
	...
	public function feof( $fp ) {
		if ( $this->has_gzip && function_exists( 'gzeof' ) ) {
			return gzeof( $fp );
		}
		return feof( $fp );
	}
	...
	public function fgets( $fp, $len = 8192 ) {
		if ( $this->has_gzip && function_exists( 'gzgets' ) ) {
	...
	public function fclose( $fp ) {
		if ( $this->has_gzip && function_exists( 'gzclose' ) ) {
			return gzclose( $fp );
		}
		return fclose( $fp );
	}
